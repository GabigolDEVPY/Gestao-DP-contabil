...
class Empresa:
                ...
                for periodo in empresa["periodos"]:
                    if tipo == "Filial":
                        if periodo["status"] == "Pendente":
                            continue
                    contas = execute_command("SELECT empresa_id, conta_status, conta_data FROM contas WHERE empresa_id = ? AND conta_data = ?", (empresa_id, periodo["empresa_data"]))
    ...
    def cadastrar_empresa(dados):
        if dados["tipo"] == "filial":
            dados.pop("tipo")
            try:
                execute_command("INSERT INTO empresas_filiais (empresa_id, id_filial, empresa_nome, empresa_cnpj) VALUES (?, ?, ?, ?)", tuple(dados.values()))
            except sqlite3.IntegrityError as e:
                if "UNIQUE constraint failed" in str(e):
                    return "Uma empresa já está cadastrada com esse ID"
        ...
        dados.pop("tipo")
        try:
            execute_command("INSERT INTO empresas (empresa_id, empresa_nome, empresa_cnpj) VALUES (?, ?, ?)", tuple(dados.values()))
        except sqlite3.IntegrityError as e:
            if "UNIQUE constraint failed" in str(e):
                return "Uma empresa já está cadastrada com esse ID"
        return 
    ...
    def criando_periodo(contas, dados, conta_id, tipo):
        for conta_original in contas:
            ...
            if conta["conta_tipo"] == tipo:
                ...
                execute_command("INSERT INTO contas (conta_nome, conta_codigo, empresa_id, conta_data) VALUES (?, ?, ?, ?)", tuple(conta.values()))
