...
companies_bp = Blueprint('empresas', __name__)




@companies_bp.route("/empresas", methods=["GET"])
def return_page():
        return render_template("empresas.html", empresas=Empresa.retornar_empresas())


@companies_bp.route("/empresas/cadastrar", methods=["POST"])
...
@companies_bp.route("/empresa/criarperiodo", methods=["POST"])
def criar_periodo():
